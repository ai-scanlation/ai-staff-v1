...
module.exports = function selectFile(query, data) {
    ...
    if (query === 'folder') {
        ...
        if (this.explorerMode === 'renameMode' || this.explorerMode === 'convertMode') {
        ...
        if (this.explorerMode === 'explorerMode') return;
        if (filesLength === 0) return;
        const firstFile = find(`tablerow[row-${foldersLength + 1}]`);
        ...
        if (query === 'all') {
            for (; i <= length; i++) {
                Vue.set(find(`tablerow[row-${i}]`).data[0], 'count', 1);
            }
        } else if (query === 'none') {
            for (; i <= length; i++) {
                Vue.set(find(`tablerow[row-${i}]`).data[0], 'count', 0);
            }
        }
    }
};